...
{
    $sql = "SELECT * FROM " . $eduOrPos . " WHERE profile_id = :prof ";
...
function insertPosition($pdo, $profileID)
{
    ...
    for($i = 1; $i <= 9; $i++)
    {
        ...
        if(isset($_SESSION[$year]) && isset($_SESSION[$desc]))
        {
            ...
            $sql = "INSERT INTO Position (profile_id, year, description) VALUES (:pid, :year, :descr)";
            $stmt = $pdo -> prepare($sql);
            $stmt -> execute(array(":pid" => $profileID,":year" => $yearVal, ":descr" => $descVal));
        }
        $rank++;
    }
}
...
function insertEducation($pdo, $profileID)
{
    ...
    for($i = 1; $i <= 9; $i++)
    {
        ...
        if(isset($_SESSION[$year]) && isset($_SESSION[$school]))
        {
            ...
            {
                $sql = "SELECT id FROM Institution WHERE name = :schoolName";
                $stmt = $pdo -> prepare($sql);
                $stmt -> execute(array(":schoolName" => $schoolVal));
